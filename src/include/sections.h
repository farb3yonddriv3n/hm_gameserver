...
#define card_name(m_dst, m_name) (m_dst && strcmp(m_dst->entity->name, m_name) == 0)
...
#define hsl_cmp_cardname(m_dst, m_src)  (strcmp(m_dst->card->entity->name, m_src) == 0)
...
#define controller_player(m_card)\
...
#define hsl_give_weapon(m_dst)\
    /* this is onboard action, no need to link destroyed wepon to defenders */\
...
/*m_dst->hero->attack = attacker->attack;\
  flag(&m_dst->hero->state, MECHANICS_ATTACK, FLAG_SET);*/
